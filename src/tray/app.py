...
def _load_domain(config):
    module = importlib.import_module(f"src.domains.{config.domain}")
    cls = getattr(module, f"{config.domain.capitalize()}Domain")
    return cls()
...
class AndrewTrayApp:
    ...
    def _create_domain(self):
        future = asyncio.run_coroutine_threadsafe(
            self._async_create_domain(), self._loop,
        )
        return future.result(timeout=10)

    async def _async_create_domain(self):
        return _load_domain(self._config)
